...
class GroqClient:
    ...
    def extract_score_from_result(self, result_raw):
        ...
        if match:
            # Extrair o valor da pontuação
            score_str = match.group(1)

            # Se houver barra, considerar apenas a parte antes dela
            if '/' in score_str:
                score_str = score_str.split('/')[0]

            # Substituir vírgula por ponto e limpar espaços
            ...
            try:
                ...
                print(f"[ERRO] Valor de pontuação inválido extraído: '{score_str}'\nResposta bruta: {result_raw}")
                return None

        print(f"[ERRO] Nenhuma pontuação encontrada na resposta: {result_raw}")
        return None
